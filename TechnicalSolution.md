这是一个非常完善的产品需求文档（PRD）。为了实现这个 InfoGather Agent，我为你设计了一套**模块化、可扩展的技术方案**。

考虑到微信生态的特殊性（个人号接入风险高、公众号主动推送限制），本方案推荐使用 **企业微信（WeChat Work）应用** 或 **测试号/服务号模板消息** 进行消息投递，同时兼顾 `itchat`（个人号）作为实验性选项。
